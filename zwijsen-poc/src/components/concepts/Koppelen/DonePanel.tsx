...
export default function DonePanel({
  ...
  return (
    ...
    >
      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-accent-violet-soft text-accent-violet">
        <Sparkles className="h-8 w-8" aria-hidden />
      </div>
      <div>
        <h2 className="text-2xl font-black text-text-primary">Klaar!</h2>
        <p className="mt-2 text-sm leading-relaxed text-text-secondary">
          Je hebt alle koppels gemaakt. Goed bezig!
        </p>
      </div>
      <div className="w-full rounded-2xl bg-surface-muted px-4 py-4 text-left">
        <dl className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="col-span-2 border-t border-border-subtle pt-3">
            ...
            <dd className="text-lg font-black text-accent-violet">{pct}% goed</dd>
          </div>
        </dl>
      </div>
      <button
        type="button"
        onClick={onRestart}
        className="flex min-h-[52px] w-full items-center justify-center gap-2 rounded-2xl bg-accent-violet px-4 py-3 text-sm font-black uppercase tracking-wide text-white shadow-md transition-opacity hover:opacity-90"
      >
