...
export default function InventoryPanel({
  ...
  return (
    <aside className="w-56 shrink-0 flex flex-col bg-white border-r border-border-subtle overflow-hidden">
      {/* Header */}
      <div className="px-4 pt-5 pb-4 border-b border-border-subtle">
        <p className="font-black text-text-primary text-sm leading-tight">
          Woordenschat
        </p>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
        {categories.map((cat) => {
          ...
          return (
            <div key={cat.id}>
              <p className="text-[10px] font-black uppercase tracking-wider text-text-muted mb-0.5">
                {cat.title}
              </p>
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        style={{
                          backgroundColor: isUsed ? Z.surfaceMuted : Z.accentInfoSoft,
                          color: isUsed ? Z.textSecondary : Z.inventoryWordTextActive,
                          border: `2px solid ${isUsed ? Z.borderSubtle : Z.inventoryWordBorderActive}`,
      ...
      {/* Legend */}
      <div className="px-4 py-4 border-t border-border-subtle space-y-1.5">
        <p className="text-[10px] font-black uppercase tracking-wider text-text-muted mb-2">
          Relaties
        </p>
        {relations.map(({ color, label }) => (
